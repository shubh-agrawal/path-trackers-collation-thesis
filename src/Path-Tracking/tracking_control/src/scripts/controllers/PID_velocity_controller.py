...
# Subscribed topic - odometry/filtered1 , cmd_vel(in km/hr), cmd_delta 
...
kd = 0.002 # differential gain
acc_thershold = 14 # threshold for acceleration
brake_threshold = 35 # threshold for brake
ki_flag = 0
ki_init = 0.6
global pub
global tar_vel
...
def callback_feedback(data):
    ...
    active_vel = last_recorded_vel*(18.0/5)
    ...
    # if error > 0.01:
    #   print ("e1")
    #   output.linear.x = (kp * error + ki * error_sum + kd * error_diff + acc_thershold)
    # if error < -0.01:
    #   print ("e2")
    ...
    if error > 0 :
        if ki_flag is 0:
            output.linear.x = kp*error + ki_init*error_sum + kd*error_diff + acc_thershold
        else:
            output.linear.x = kp*error + ki*error_sum + kd*error_diff + acc_thershold
    ...
    if error < -1 :
        if ki_flag is 0:
            ki_flag = 1
        output.linear.x = 4*(kp*error + ki*error_sum + kd*error_diff) - brake_threshold
    ...
    # Thresholding the steering angle between 40 degrees and -40 degrees for mahindra
    # it is -30 to 30 for prius

    output.angular.z = min(40.0, max(-40.0, tar_delta))
