...
def callback_feedback(data):
    ...
    if len(x_p.poses) > 0:
        ...
        cross_err.linear.y = (-1) * (state.yaw - path_angle)
        cross_err.linear.z = path_length[ind]

        delta_lqr, throttle  = lqr_steering_control(state, e, e_th, pe, pe_th, x_p) 
        cmd_steer = Twist()
        cmd_steer.linear.x = min(1,max(throttle,-1))
        cmd_steer.angular.z = min(40,max(delta_lqr*180.0/math.pi,-40))
        pub1.publish(cross_err)
...
def main():
    ...
    rospy.Subscriber("/odometry/filtered1",Odometry, callback_feedback, queue_size=1)
    rospy.Subscriber("astroid_path",Path, callback_path, queue_size=1)
    rospy.Subscriber("cmd_vel", Twist, callback_vel)
    pub = rospy.Publisher(pid_output, Twist, queue_size=1)
    pub1 = rospy.Publisher('cross_track_error', Twist, queue_size = 10)
