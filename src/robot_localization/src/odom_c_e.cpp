...
int main(int argc, char** argv){
  ros::init(argc, argv, "odom_corr_e");

  ros::NodeHandle n;
  //odometry - covariance + tf
  ros::Subscriber odom_sub=n.subscribe<nav_msgs::Odometry>("/odom_imu_combined_e",50,odomCallback);
  odom_pub = n.advertise<nav_msgs::Odometry>("/odom_c_e", 50);
  ros::spin();
  return 0;
}