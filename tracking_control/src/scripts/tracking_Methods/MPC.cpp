...
const double Lf = 1.0;
double ref_v = 10; //Works up to 110 ;)
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      fg[0] += 250*CppAD::pow((vars[delta_start + t]/(0.5236*Lf))*vars[a_start + t], 2);
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      //delta from simulator is negative
      fg[1 + psi_start + t] = psi1 - (psi0 + v0 * delta0 / Lf * dt);
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
      //simulator delta is negative
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.5236*Lf;
    vars_upperbound[i] = 0.5236*Lf;
  }
